...
unsigned gamepad(int id) {
    ...
    if( id == 0 ) {
        GamepadUpdate();
        old[0] = pad[0], pad[0] = gamepad_update(GAMEPAD_0+0, 0, 0);
        old[1] = pad[1], pad[1] = gamepad_update(GAMEPAD_1+0, 0, 0);
        old[2] = pad[2], pad[2] = gamepad_update(GAMEPAD_2+0, 0, 0);
        old[3] = pad[3], pad[3] = gamepad_update(GAMEPAD_3+0, 0, 0);
        return 0;
    }
