...
void input() {
    ...
    if( ZX_JOYSTICKS_INVERTED[G] & 1 ) T=L, L=R, R=T;
    if( ZX_JOYSTICKS_INVERTED[G] & 2 ) T=U, U=D, D=T;
    ...
    for(int i = 0; i < 4; ++i) {
        ...
        // set joystick inputs
        int L = pad&1, R = pad&2, U = pad&4, D = pad&8, F = !!(pad&16), A = ZX_JOYSTICKS_AUTOFIRE_BUTTON[G], T = 0;
        if( ZX_JOYSTICKS_INVERTED[G] & 1 ) T=L, L=R, R=T;
        if( ZX_JOYSTICKS_INVERTED[G] & 2 ) T=U, U=D, D=T;
        ZXJoysticks(G,!!U,!!D,!!L,!!R,F ^ (F && ZX_JOYSTICKS_AUTOFIRE[G] ? A : 0));
...
//        if( ui_click(rmb_held*23+"- Magic button (NMI) -\0- Magic button (NMI) -\nGenerates a Non-Maskable Interrupt", "") ) cmdkey = 'NMI';
        ...
        if( ui_click(rmb_held*20+"- Loading options -\0- Toggle FastMedia -\n0:off, 1:faster media loading", TAPE_STR "\f%d", ZX_FASTTAPE )) if(rmb_up) cmdkey = 'FAST'; else
        {
            ...
            */
            ...
            // @todo: eject media without boot

            ui_mirror=1;
            ui_dialog_option(1|4,   REWIND_STR,"-Reload media-",'BOOT',ZX_MEDIA);
            ui_dialog_option(1|4,   tape_playing() ? STOP_STR : PLAY_STR, tape_playing() ? "-Stop media-" : "-Play media-",tape_playing() ? 'STOP':'PLAY',0);
            if(ZX_MEDIA[0] == '#')
            ui_dialog_option(1,     EJECT_STR,"-Change media-",'LIST',ZX_MEDIA);
            else
            ui_dialog_option(1,     EJECT_STR,"-Insert media-",'GAME',0);
//          ui_dialog_option(1,     EJECT_STR,"-Eject media-",'WIPE',0);
    ...
    if( changed ) {
        ...
        switch(cmdkey_) { default: 
            ...
            break; case 'SCAN': {
                                    ...
                                    rescan( f ? f : cwd() ), /*browser = !!numgames,*/ ui_dialog_new(NULL);
                                }
            ...
            break; case 'LIST': { // cmdarg in "#ID" format. @todo: add LIST redefineable cmd
                ...
                if( count > 0 ) {
                    ...
                    //ui_dialog_option(1|4,EJECT_STR,"-Eject media-",'WIPE',0);
            ...
            break; case 'JOY0': {
                ...
                if( ZX_PLAYERNUM ) {
                    UI_JOYSTICK("Invert X axis",ZX_JOYSTICKS_INVERTED[ZX_PLAYERNUM]&1,"^1","^1", "-Toggle horizontal movement direction-");
                    UI_JOYSTICK("Invert Y axis",ZX_JOYSTICKS_INVERTED[ZX_PLAYERNUM]&2,"^2","^2", "-Toggle vertical movement direction-");
                } else {
                    ui_dialog_separator();
                    ui_dialog_separator();
                }
                ...
                ui_dialog_separator();
                ui_dialog_checkbox2(&ZX_HORACE, ZX_PLAYERNUM ? "\f\f" : "\f\f ", "- Keep Horace screenmate around while playing -", NULL );
                if( ZX_PLAYERNUM > 0 )
...
int gui(const char *status) {
    ...
    if( ZX_HORACE || cmdkey == 'JOY0' || cmdkey == 'JOY1' ) {
        ...
        if( !G ) pad = 0
            | PAD_L * key_held(TK_LEFT) 
            | PAD_R * key_held(TK_RIGHT) 
            | PAD_U * key_held(TK_UP) 
            | PAD_D * key_held(TK_DOWN) 
            | PAD_A * key_held(TK_TAB);
